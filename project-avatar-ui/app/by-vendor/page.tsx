...
const RecruiterByVendor = () => {
  ...
  const fetchRecruiters = async (page: number) => {
    try {
      const response = await axios.get(`${API_URL}/recruiters/by-vendor`, {
        params: { page, pageSize, type: "vendor" },
        headers: { AuthToken: localStorage.getItem("token") },
      });
      setRowData(response.data.data);
      setTotalPages(Math.min(response.data.pages, 307));
  ...
  const handleDeleteRow = async () => {
    if (gridRef.current) {
      ...
      if (selectedRows.length > 0) {
        ...
        try {
          await axios.delete(`${API_URL}/recruiters/byVendor/remove/${recruiterId}`, {
            headers: { AuthToken: localStorage.getItem("token") },
          });
          setAlertMessage("Recruiter deleted successfully.");
          fetchRecruiters(currentPage);
  ...
  return (
    <div className="p-4 mt-20 mb-10 ml-20 mr-20 bg-gray-100 rounded-lg shadow-md relative">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-3xl font-bold text-gray-800">
          Recruiter Management (By Vendor)
        </h1>
        <div className="flex space-x-2">
          <button
            onClick={handleAddRow}
