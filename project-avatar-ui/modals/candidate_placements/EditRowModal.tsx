...
interface Placement {
  ...
  projectdocs: string;
  notes: string;
  masteragreementid: number;  // Changed from optional to required
  otheragreementsids?: string;
}
...
const EditRowModal: React.FC<EditRowModalProps> = ({ isOpen, onClose, placement, onSave }) => {
  ...
  useEffect(() => {
    if (isOpen && placement) {
      setFormData({ 
        ...placement,
        masteragreementid: placement.masteragreementid || 0  // Set default to 0 if undefined
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await axios.put(`${API_URL}/candid/placements/update/${formData.id}`, {
        ...formData,
        masteragreementid: formData.masteragreementid || 0  // Ensure 0 is sent if undefined
      }, {
        headers: { AuthToken: localStorage.getItem("token") },
      });
