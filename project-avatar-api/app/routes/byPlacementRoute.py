...
from app.database.db import get_db
from app.schemas import RecruiterCreate, RecruiterUpdate, RecruiterResponse
from app.controllers.byPlacementController import (
    get_recruiters_by_client_placement,
    add_recruiter,
    update_recruiter,
    delete_recruiter
)
...
@router.get("/recruiters/by-client-placement", response_model=dict)
def read_recruiters_by_client_placement(
    page: int = Query(1, alias="page"),
    page_size: int = Query(1000, alias="pageSize"),
    search: Optional[str] = Query(None),
    db: Session = Depends(get_db)
):
    return get_recruiters_by_client_placement(db, page, page_size, search)

@router.post("/recruiters/by-client-placement/add", response_model=RecruiterResponse)
...
@router.put("/recruiters/by-client-placement/update/{recruiter_id}", response_model=RecruiterResponse)
...
@router.delete("/recruiters/by-client-placement/remove/{recruiter_id}")
