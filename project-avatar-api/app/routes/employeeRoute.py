...
@router.put("/employee/update/{employee_id}", response_model=Dict)
...
def get_manager_options_route(db: Session = Depends(get_db)):
    return get_manager_options(db)

@router.get("/employee/options/designations", response_model=List[Dict[str, str]])
def get_designation_options_route(db: Session = Depends(get_db)):
    return get_designation_options(db)
