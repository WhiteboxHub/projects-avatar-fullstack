...
# In app/controllers/bymonthController.py
...
def get_invoices_grouped_by_month(
    ...
    for row in rows:
        ...
        invoice_data = {k: v for k, v in row.items()}
        grouped_data[key]["invoices"].append(invoice_data)
...
def get_pname_list(db: Session):
    query = text("""
        ...
        WHERE c.candidateid = p.candidateid 
        AND o.placementid = p.id 
        AND p.vendorid = v.id 
        AND p.clientid = cl.id 
        ORDER BY pname
...
def delete_invoice(db: Session, invoice_id: int):
    ...
    invoice.status = "Delete"  # Soft delete by changing status
    db.commit()
    return {"message": "Invoice deleted successfully"}