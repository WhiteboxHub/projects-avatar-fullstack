...
class VendorController:
    ...
    async def get_vendors(self, page: int, page_size: int, search: Optional[str] = None):
        ...
        for vendor in query.all():
            ...
            vendor_dict['tier'] = str(vendor_dict['tier'])  # Convert tier to string
    ...
    async def add_vendor(self, vendor_data: VendorCreate) -> VendorInDB:
        ...
        new_vendor_dict = new_vendor.__dict__
        new_vendor_dict['tier'] = str(new_vendor_dict['tier'])  # Convert tier to string
        return VendorInDB(**new_vendor_dict)
