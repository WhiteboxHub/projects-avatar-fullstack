# controllers/byVendorController.py
...
def get_recruiters_by_vendor(
    ...
    """
    Get vendors with pagination and their recruiters
    """
    if type == "vendor":
...
def create_vendor_recruiter(db: Session, recruiter: RecruiterCreate):
...
def update_vendor_recruiter(db: Session, recruiter_id: int, recruiter: RecruiterUpdate):
...
def delete_vendor_recruiter(db: Session, recruiter_id: int):
    db_recruiter = db.query(Recruiter).filter(Recruiter.id == recruiter_id).first()
    if not db_recruiter:
        raise HTTPException(status_code=404, detail="Recruiter not found")
...
def get_vendor_recruiter(db: Session, recruiter_id: int):
