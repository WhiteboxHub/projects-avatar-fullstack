...
from app.schemas import VendorCreate, VendorUpdate, VendorResponse, VendorSearchBase
...
class VendorController:
    ...
    async def get_vendors(self, page: int, page_size: int, search: Optional[str] = None):
        query = self.db.query(
            ...
            Vendor.solicited,
            Vendor.minrate,
            Vendor.hirebeforeterm,
            Vendor.hireafterterm,
            Vendor.latepayments,
            Vendor.totalnetterm,
            Vendor.defaultedpayment,
            ...
            Vendor.manageremail,
            Vendor.managerphone,
            Vendor.secondaryname,
            Vendor.secondaryemail,
            Vendor.secondaryphone,
            Vendor.timesheetemail,
            Vendor.agreementname,
            Vendor.agreementlink,
            Vendor.subcontractorlink,
            Vendor.nonsolicitationlink,
            Vendor.nonhirelink,
            Vendor.clients,
            Vendor.notes
        )
        ...
        vendors = [VendorResponse.from_orm(vendor) for vendor in query.all()]
    ...
    async def add_vendor(self, vendor_data: VendorCreate) -> VendorResponse:
    ...
    async def edit_vendor(self, vendor_id: int, vendor_data: VendorUpdate) -> VendorResponse:
        vendor = self.db.query(Vendor).filter(Vendor.id == vendor_id).first()
        if not vendor:
            raise HTTPException(status_code=404, detail="Vendor not found")
