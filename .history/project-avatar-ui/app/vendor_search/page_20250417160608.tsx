                ...
                <ul>
                  {vendors.map((vendor) => (
                    <li key={vendor.id} className="mb-4">
                      ...
                      <Dropdown
                        title="Manager"
                        isOpen={openAccordion[String(vendor.id)] === "manager"}
                        onClick={() => toggleAccordion(String(vendor.id), "manager")}
                      >
                        <p className="font-medium">
                          <strong>Manager Name:</strong> {vendor.managerName}
                        </p>
                        <p className="font-medium">
                          <strong>Manager Email:</strong> {vendor.managerEmail}
                        </p>
                      </Dropdown>

                      <Dropdown
                        title="HR"
                        isOpen={openAccordion[String(vendor.id)] === "hr"}
                        onClick={() => toggleAccordion(String(vendor.id), "hr")}
                      >
                        <p className="font-medium">
                          <strong>HR Name:</strong> {vendor.hrName}
                        </p>
                        <p className="font-medium">
                          <strong>HR Email:</strong> {vendor.hrEmail}
                        </p>
                      </Dropdown>

                      <Dropdown
                        title="Agreements"
                        isOpen={openAccordion[String(vendor.id)] === "agreements"}
                        onClick={() => toggleAccordion(String(vendor.id), "agreements")}
                      >
                        <p className="font-medium">{vendor.agreements}</p>
                      </Dropdown>

                      <Dropdown
                        title="Requirements"
                        isOpen={openAccordion[String(vendor.id)] === "requirements"}
                        onClick={() => toggleAccordion(String(vendor.id), "requirements")}
                      >
                        <p className="font-medium">{vendor.requirements}</p>
                      </Dropdown>
