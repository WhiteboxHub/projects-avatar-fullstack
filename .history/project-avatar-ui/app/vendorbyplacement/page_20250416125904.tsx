...
const RecruiterByVendorPlacement = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/by/recruiters/by-vendor-placement`,
        {
          params: {
            page: currentPage,
            pageSize: pageSize,
    ...
    () => [
      {
        ...
        cellRenderer: (params: any) => {
          if (params.data.isGroupRow) {
            ...
            const vendor = vendors.find(c => c.vendorid === params.data.vendorid);
            return (
  ...
  const handleAdd = async (formData: RecruiterData) => {
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/by/recruiters/by-vendor-placement/add`,
        formData
      );
  ...
  const handleEdit = async (formData: RecruiterData) => {
    try {
      await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/by/recruiters/by-vendor-placement/update/${formData.id}`,
        formData
      );
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this recruiter?")) {
      try {
        await axios.delete(
          `${process.env.NEXT_PUBLIC_API_URL}/by/recruiters/by-vendor-placement/remove/${id}`
        );
  ...
  const handleDownloadPDF = () => {
    ...
    doc.save("recruiters-by-vendor-placement.pdf");
  };
  ...
  return (
    <div className="p-4 mt-20 mb-10 ml-20 mr-20 bg-gray-100 rounded-lg shadow-md relative">
      ...
      <AddRowModal
        ...
        onSubmit={handleAdd}
        vendorOptions={vendorOptions.map(v => ({ id: v.id, name: v.name }))}
      />
      ...
      <EditRowRecruiter
        ...
        defaultVendorId={selectedVendorId || modalState.selectedRow?.vendorid || 0}
      />
