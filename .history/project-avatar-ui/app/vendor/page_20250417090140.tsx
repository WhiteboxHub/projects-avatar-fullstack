...
import AddRowModal from "@/modals/recruiter_byVendorDetailed_modals/AddRowRecruiter";
import EditRowModal from "@/modals/recruiter_byVendorDetailed_modals/EditRowRecruiter";
import ViewRowModal from "@/modals/recruiter_byVendorDetailed_modals/ViewRowRecruiter";
...
import { RecruiterDetails } from "@/types/byVendorDetailed";
...
const RecruiterByVendorDetailed = () => {
  ...
  const [vendorList, setVendorList] = useState<Company[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/recruiters/by-vendor-detailed`,
        {
          params: {
            page: currentPage,
            pageSize: pageSize,
  ...
  const handleAdd = async (formData: RecruiterData) => {
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/recruiters/by-vendor-detailed/add`,
        formData
      );
  ...
  const handleEdit = async (formData: RecruiterData) => {
    try {
      await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/recruiters/by-vendor-detailed/update/${formData.id}`,
        formData
      );
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this recruiter?")) {
      try {
        await axios.delete(
          `${process.env.NEXT_PUBLIC_API_URL}/recruiters/by-vendor-detailed/remove/${id}`
        );
  ...
  const handleDownloadPDF = () => {
    ...
    doc.save("recruiters-by-vendor-detailed.pdf");
  };
...
export default RecruiterByVendorDetailed;
